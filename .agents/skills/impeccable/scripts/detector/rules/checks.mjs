...
function buildHtmlPatternCorpora(html) {
  ...
  return { classText: classParts.join("\n"), styleText: styleParts.join("\n") };
}
...
function resolveBackgroundInfo(el, win, customPropMap) {
  ...
  while (current && current.nodeType === 1) {
    ...
    if (
      ...
      bg =
        parseRgb(style.color) || parseColorResolved(style.color, customPropMap);
    }
...
function checkElementPseudoStripeDOM(el) {
  ...
  for (const which of ["::before", "::after"]) {
    ...
    if ((Number.parseFloat(ps.opacity) || 0) <= 0.01 || ps.display === "none") {
      continue;
    }
