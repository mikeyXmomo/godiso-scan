    ...
    list?.forEach?.((node) => {
      if (node.type === "Rule" && node.block) {
        ...
        for (const selector of splitCssList(selectorText)) {
          ...
          if (isHover) {
            matchSelector = selector.replaceAll(/:hover\b/gi, "").trim();
            if (!matchSelector || /[>+~]\s*$/.test(matchSelector)) {
              matchSelector = null;
            } else {
              matchSelector = matchSelector.replaceAll(
                /(^|[\s>+~])(?=$|[\s>+~])/g,
                "$1*"
              );
            }
...
function resolveLinkedCssPath(fileDir, href) {
  ...
  if (!rootRelative) {
    return path.resolve(fileDir, stripped);
  }
  ...
  if (segments.some((p) => p === "..")) {
    return path.join(
      fileDir,
      segments.filter((p) => p !== "..").join(path.sep)
    );
  }
  ...
  for (;;) {
    ...
    if (parent === dir) {
      break;
    } // never use the filesystem root as document root
    try {
