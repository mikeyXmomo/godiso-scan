...
function scanToBlockOrStatementEnd(text, from) {
  ...
  while (i < text.length) {
    ...
    if (quote) {
      ...
      return { index: i, kind: "block" };
    } else if (ch === ";") {
      return { index: i, kind: "statement" };
    }
    i++;
  }
...
export function reconcileCss(existingCss, variantCss) {
    ...
    for (const node of incomingNodes) {
      ...
      if (node.type === "rule") {
        ...
        if (match) {
          ...
          atIndex.set(key, existingNodes.at(-1));
          appended++;
        }
      } else {
        existingNodes.push({ ...node });
  ...
  return { appended, css: serializeNodes(existing), replaced };
}
...
export function bakeParamValues(css, params = [], values = {}) {
  ...
  const bakeBody = (body) => {
    ...
    return out
      .replaceAll(/(^|;)\s*--impeccable-variant-ready\s*:[^;{}]*/g, "$1")
      .replaceAll(/;\s*;/g, ";")
      .replace(/^\s*;\s*/, "");
  };
  ...
  const transform = (list) => {
    ...
    for (const node of list) {
      ...
      result.push({ ...node, body, prelude: kept.join(", ") });
    }
    return result;
  };
...
export function pruneUnusedSelectors(
  ...
  for (let pass = 0; pass < 3; pass++) {
    ...
    try {
      ...
      return { removed, source }; // never let pruning break an accept
    }
...
function removeSelectorAt(source, start, end) {
  ...
  return {
    ...
    selector: target,
    source: `${
      source.slice(0, preludeStart) + indent + kept.join(", ")
    } ${source.slice(braceIdx)}`,
  };
}
